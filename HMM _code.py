...
def _identity(labeled_symbols):   #Karina - returns the same value that was used as its argument
...
class HiddenMarkovModelTagger(TaggerI):
    ...
    :param transform: an optional function for transforming training
        instances, defaults to the identity function. (Karina - returns the same value that was used as its argument)
    ...
    @classmethod
    def _train(cls, labeled_sequence, test_sequence=None,            #cls = vs. self, for the first argument to class methods
...
def demo_bw():
    ...
    """making training data for tagger using just one csv file for now:
"""
...
for i in range(0,len(trans.utterances)):
    lemma_tuple = trans.utterances[i].pos_lemmas(wn_lemmatize=True)
    lemmatuplelist.append(lemma_tuple)

"""playing around with tagger:
"""
...
print (tagger.tag("My name is Karina O'Neill.".split()))
