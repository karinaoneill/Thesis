...
"""
Want to create a CSV file named perfectmatchdata.csv in which each utterance utt has its own row
...
print ("Creating perfectmatchdata.csv from swda corpus")

csvwriter = csv.writer(open('perfectmatchdata.csv', 'wt'))
...
for utt in corpus.iter_utterances(display_progress=True):
    if utt.tree_is_perfect_match() and utt.damsl_act_tag() != '+':
...
print ("perfectmatchdata.csv created")

import pandas as pd 
frame = pd.read_csv('perfectmatchdata.csv',index_col=None, header=0)
...
a = J_train['conversation_no'].as_matrix()
...
c_other_features_train = np.column_stack((a,b,c,d)) 
       
c_train_data_featurescopy = c_train_data_features.copy()

c_train_data_features_all = np.hstack((c_other_features_train,c_train_data_featurescopy))
...
ti_other_features_train = np.column_stack((a,b,c,d)) 
       
ti_train_data_featurescopy = ti_train_data_features.copy()

ti_train_data_features_all = np.hstack((ti_other_features_train,ti_train_data_featurescopy))
...
# four non-BoWs features, stacked together:
e = J_test['conversation_no'].as_matrix()
f = J_test['caller'].as_matrix()
g = J_test['sent_length'].as_matrix()
h = J_test['rootnode_num'].as_matrix()
other_features_test = np.column_stack((e,f,g,h))            
...
#
print ("Training the MLPs (this may take a while)...")
...
true_labels = np.asarray(c_targetlist2)

# weighted accuracies for the decision tree:
cprecisionDTw, crecallDTw, cfscoreDTw, csupportDTw = precision_recall_fscore_support(true_labels,c_pred_cls2,average='weighted')
tprecisionDTw, trecallDTw, tfscoreDTw, tsupportDTw = precision_recall_fscore_support(true_labels,ti_pred_cls2,average='weighted')

# unweighted accuracies for the decision tree:
cprecisionDTu, crecallDTu, cfscoreDTu, csupportDTu = precision_recall_fscore_support(true_labels,c_pred_cls2)
tprecisionDTu, trecallDTu, tfscoreDTu, tsupportDTu = precision_recall_fscore_support(true_labels,ti_pred_cls2)

# weighted accuracies for the random forest:
cprecisionRFw, crecallRFw, cfscoreRFw, csupportRFw = precision_recall_fscore_support(true_labels,c_pred_cls3,average='weighted')
tprecisionRFw, trecallRFw, tfscoreRFw, tsupportRFw = precision_recall_fscore_support(true_labels,ti_pred_cls3,average='weighted')

# unweighted accuracies for the random forest:
cprecisionRFu, crecallRFu, cfscoreRFu, csupportRFu = precision_recall_fscore_support(true_labels,c_pred_cls3)
tprecisionRFu, trecallRFu, tfscoreRFu, tsupportRFu = precision_recall_fscore_support(true_labels,ti_pred_cls3)

# weighted accuracies for the MLP:
cprecisionMLPw, crecallMLPw, cfscoreMLPw, csupportMLPw = precision_recall_fscore_support(true_labels,c_pred_cls4,average='weighted')
tprecisionMLPw, trecallMLPw, tfscoreMLPw, tsupportMLPw = precision_recall_fscore_support(true_labels,ti_pred_cls4,average='weighted')

# unweighted accuracies for the MLP:
cprecisionMLPu, crecallMLPu, cfscoreMLPu, csupportMLPu = precision_recall_fscore_support(true_labels,c_pred_cls4)
tprecisionMLPu, trecallMLPu, tfscoreMLPu, tsupportMLPu = precision_recall_fscore_support(true_labels,ti_pred_cls4)

# weighted accuracies for the NB:
cprecisionNBw, crecallNBw, cfscoreNBw, csupportNBw = precision_recall_fscore_support(true_labels,c_pred_cls9,average='weighted')
tprecisionNBw, trecallNBw, tfscoreNBw, tsupportNBw = precision_recall_fscore_support(true_labels,ti_pred_cls9,average='weighted')

# unweighted accuracies for the NB:
cprecisionNBu, crecallNBu, cfscoreNBu, csupportNBu = precision_recall_fscore_support(true_labels,c_pred_cls9)
tprecisionNBu, trecallNBu, tfscoreNBu, tsupportNBu = precision_recall_fscore_support(true_labels,ti_pred_cls9)

# weighted accuracies for the Majority Most Frequent classifier:
cprecisionMFw, crecallMFw, cfscoreMFw, csupportMFw = precision_recall_fscore_support(true_labels,c_pred_cls7,average='weighted')
tprecisionMFw, trecallMFw, tfscoreMFw, tsupportMFw = precision_recall_fscore_support(true_labels,ti_pred_cls7,average='weighted')

# unweighted accuracies for the Majority Most Frequent classifier:
cprecisionMFu, crecallMFu, cfscoreMFu, csupportMFu = precision_recall_fscore_support(true_labels,c_pred_cls7)
tprecisionMFu, trecallMFu, tfscoreMFu, tsupportMFu = precision_recall_fscore_support(true_labels,ti_pred_cls7)

# weighted accuracies for the SVM:
cprecisionSVMw, crecallSVMw, cfscoreSVMw, csupportSVMw = precision_recall_fscore_support(true_labels,c_pred_cls8,average='weighted')
tprecisionSVMw, trecallSVMw, tfscoreSVMw, tsupportSVMw = precision_recall_fscore_support(true_labels,ti_pred_cls8,average='weighted')

# unweighted accuracies for the SVM:
cprecisionSVMu, crecallSVMu, cfscoreSVMu, csupportSVMu = precision_recall_fscore_support(true_labels,c_pred_cls8)
tprecisionSVMu, trecallSVMu, tfscoreSVMu, tsupportSVMu = precision_recall_fscore_support(true_labels,ti_pred_cls8)
...
c_DTconfmatrix = confusion_matrix(true_labels,c_pred_cls2)
c_RFconfmatrix = confusion_matrix(true_labels,c_pred_cls3)
c_MLPconfmatrix = confusion_matrix(true_labels,c_pred_cls4)
c_NBconfmatrix = confusion_matrix(true_labels,c_pred_cls9)
c_MFconfmatrix = confusion_matrix(true_labels,c_pred_cls7)
c_SVMconfmatrix = confusion_matrix(true_labels,c_pred_cls8)

ti_DTconfmatrix = confusion_matrix(true_labels,ti_pred_cls2)
ti_RFconfmatrix = confusion_matrix(true_labels,ti_pred_cls3)
ti_MLPconfmatrix = confusion_matrix(true_labels,ti_pred_cls4)
ti_NBconfmatrix = confusion_matrix(true_labels,ti_pred_cls9)
ti_MFconfmatrix = confusion_matrix(true_labels,ti_pred_cls7)
ti_SVMconfmatrix = confusion_matrix(true_labels,ti_pred_cls8)
...
print("* root_node counts - top 10:", frame["root_node"].value_counts().nlargest(10), sep="\n")
...
#Majority - most_freq
c_MajFreq_cv5 = cross_val_score(c_MajFreq,c_test_data_features_all,true_labels,cv=5)
print ("MajFreq countvec - 5-fold CV:", c_MajFreq_cv5)
c_MajFreq_cv10 = cross_val_score(c_MajFreq,c_test_data_features_all,true_labels,cv=10)
print ("MajFreq countvec - 10-fold CV:", c_MajFreq_cv10)
ti_MajFreq_cv5 = cross_val_score(ti_MajFreq,ti_test_data_features_all,true_labels,cv=5)
print ("MajFreq tfidfvec - 5-fold CV:", ti_MajFreq_cv5)
ti_MajFreq_cv10 = cross_val_score(ti_MajFreq,ti_test_data_features_all,true_labels,cv=10)
print ("MajFreq tfidfvec - 10-fold CV:", ti_MajFreq_cv10)

#Decision Tree
c_dtree_cv5 = cross_val_score(c_dtree,c_test_data_features_all,true_labels,cv=5)
print ("dtree countvec - 5-fold CV:", c_dtree_cv5)
c_dtree_cv10 = cross_val_score(c_dtree,c_test_data_features_all,true_labels,cv=10)
print ("dtree countvec - 10-fold CV:", c_dtree_cv10)
ti_dtree_cv5 = cross_val_score(ti_dtree,ti_test_data_features_all,true_labels,cv=5)
print ("dtree tfidfvec - 5-fold CV:", ti_dtree_cv5)
ti_dtree_cv10 = cross_val_score(ti_dtree,ti_test_data_features_all,true_labels,cv=10)
print ("dtree tfidfvec - 10-fold CV:", ti_dtree_cv10)

#Random Forest
c_forest_cv5 = cross_val_score(c_forest,c_test_data_features_all,true_labels,cv=5)
print ("forest countvec - 5-fold CV:", c_forest_cv5)
c_forest_cv10 = cross_val_score(c_forest,c_test_data_features_all,true_labels,cv=10)
print ("forest countvec - 10-fold CV:", c_forest_cv10)
ti_forest_cv5 = cross_val_score(ti_forest,ti_test_data_features_all,true_labels,cv=5)
print ("forest tfidfvec - 5-fold CV:", ti_forest_cv5)
ti_forest_cv10 = cross_val_score(ti_forest,ti_test_data_features_all,true_labels,cv=10)
print ("forest tfidfvec - 10-fold CV:", ti_forest_cv10)

#MLP
c_MLP_cv5 = cross_val_score(c_MLP,c_test_data_features_all,true_labels,cv=5)
print ("MLP countvec - 5-fold CV:", c_MLP_cv5)
c_MLP_cv10 = cross_val_score(c_MLP,c_test_data_features_all,true_labels,cv=10)
print ("MLP countvec - 10-fold CV:", c_MLP_cv10)
ti_MLP_cv5 = cross_val_score(ti_MLP,ti_test_data_features_all,true_labels,cv=5)
print ("MLP tfidfvec - 5-fold CV:", ti_MLP_cv5)
ti_MLP_cv10 = cross_val_score(ti_MLP,ti_test_data_features_all,true_labels,cv=10)
print ("MLP tfidfvec - 10-fold CV:", ti_MLP_cv10)

#Naive Bayes
c_NB_cv5 = cross_val_score(c_NB,c_test_data_features_all,true_labels,cv=5)
print ("NB countvec - 5-fold CV:", c_NB_cv5)
c_NB_cv10 = cross_val_score(c_NB,c_test_data_features_all,true_labels,cv=10)
print ("NB countvec - 10-fold CV:", c_NB_cv10)
ti_NB_cv5 = cross_val_score(ti_NB,ti_test_data_features_all,true_labels,cv=5)
print ("NB tfidfvec - 5-fold CV:", ti_NB_cv5)
ti_NB_cv10 = cross_val_score(ti_NB,ti_test_data_features_all,true_labels,cv=10)
print ("NB tfidfvec - 10-fold CV:", ti_NB_cv10)

#Linear SVM
c_SVM_cv5 = cross_val_score(c_SVM,c_test_data_features_all,true_labels,cv=5)
print ("SVM countvec - 5-fold CV:", c_SVM_cv5)
c_SVM_cv10 = cross_val_score(c_SVM,c_test_data_features_all,true_labels,cv=10)
print ("SVM countvec - 10-fold CV:", c_SVM_cv10)
ti_SVM_cv5 = cross_val_score(ti_SVM,ti_test_data_features_all,true_labels,cv=5)
print ("SVM tfidfvec - 5-fold CV:", ti_SVM_cv5)
ti_SVM_cv10 = cross_val_score(ti_SVM,ti_test_data_features_all,true_labels,cv=10)
